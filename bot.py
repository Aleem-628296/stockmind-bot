...
def button_handler(query):
    ...
    try:
        if callback_data == "main_menu":
            ...
            conn = get_db()
            if not conn:
                edit_message(chat_id, message_id, "❌ Database error. Please try again.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
                return
            ...
            conn = get_db()
            if not conn:
                edit_message(chat_id, message_id, "❌ Database error.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
                return
            c = conn.cursor(cursor_factory=RealDictCursor)
        ...
        elif callback_data.startswith("s_"):  # Changed from "sellitem_"
        ...
        elif callback_data.startswith("sw_"):  # sell_walkin: sw_{id}_{qty}
        ...
        elif callback_data.startswith("st_"):  # sell_type: st_{id}_{qty}
            parts = callback_data.split("_")
            item_id = int(parts[1])
            qty = int(parts[2])
        ...
        elif callback_data.startswith("a_"):  # Changed from "additem_"
            item_id = int(callback_data[2:])
            conn = get_db()
            if not conn:
                edit_message(chat_id, message_id, "❌ Database error.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
        ...
        elif callback_data.startswith("selcat_"):  # select category for new item
            category = callback_data[7:]
            state, data_dict = get_state(chat_id)
        ...
        elif callback_data.startswith("e_"):  # Changed from "edititem_"
            item_id = int(callback_data[2:])
            conn = get_db()
            if not conn:
                edit_message(chat_id, message_id, "❌ Database error.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
        ...
        elif callback_data.startswith("del_"):  # delete item
        ...
        elif callback_data.startswith("cdel_"):  # confirm delete
        ...
        edit_message(chat_id, message_id, "❌ An error occurred. Please try again.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})

def process_sale_confirmation(chat_id, item_id, qty, customer_info, message_id=None):
    conn = get_db()
    if not conn:
        send_message(chat_id, "❌ Database error. Please try again.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
        return
    ...
    try:
        ...
        send_message(chat_id, "❌ Error recording sale. Please try again.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
        conn.close()
        clear_state(chat_id)
        return
...
def handle_text_message(chat_id, text):
    ...
    state, data_dict = get_state(chat_id)
    print(f"📝 Text received: '{text}' | State: {state}")

    try:
        if state and state.startswith("sell_qty_"):
            ...
            conn = get_db()
            if not conn:
                send_message(chat_id, "❌ Database error.", reply_markup={"inline_keyboard": [[{"text": "🏠 Main Menu", "callback_data": "main_menu"}]]})
                return
            c = conn.cursor(cursor_factory=RealDictCursor)
            c.execute("SELECT * FROM stock WHERE id=%s", (item_id,))
            item = c.fetchone()
        ...
        elif state and state.startswith("add_qty_"):
            item_id = int(state.split("_")[2])
            try:
                qty = int(text)
                if qty <= 0: raise ValueError
